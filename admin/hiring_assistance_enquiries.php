...
require_admin_login();
require_permission('can_view_hiring_assistance');
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $del = $mysqli->prepare('DELETE FROM hiring_assistance_submissions WHERE id = ?');
    $del->bind_param('i', $id);
    $del->execute();
    $del->close();
    header('Location: hiring_assistance_enquiries.php?deleted=1');
    exit;
}

$countRow = $mysqli->query('SELECT COUNT(*) AS c FROM hiring_assistance_submissions')->fetch_assoc();
$todayRow = $mysqli->query('SELECT COUNT(*) AS c FROM hiring_assistance_submissions WHERE DATE(created_at)=CURDATE()')->fetch_assoc();
$total = (int)($countRow['c'] ?? 0);
$today = (int)($todayRow['c'] ?? 0);

$result = $mysqli->query('
    SELECT id, name, phone, email, organization_name, resources_salary, created_at
    FROM hiring_assistance_submissions
    ORDER BY created_at DESC
');

$admin_name = htmlspecialchars($_SESSION['admin_username'] ?? 'Admin');
$adminNavActive = 'hiring_assistance';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Hiring Assistance Enquiries — Elevate Pro</title>
    ...
    <style>
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .toast{background:rgba(91,141,239,.14);border:1px solid rgba(91,141,239,.35);color:#c8dcff;padding:10px 12px;border-radius:10px;margin-bottom:14px}
        ...
        .detail{white-space:pre-wrap;max-width:420px;color:#d7d7e2;line-height:1.45}
...
<div class="sidebar-footer">
    ...
    <main class="main">
        <div class="top">
            <div>
                <div class="title">Hiring Assistance Enquiries</div>
                <div class="sub">Submissions from the Hiring Assistance form (For Business)</div>
            </div>
        ...
        <div class="content">
            ...
            <div class="stats">
                <div class="card">
                    <div class="label">Total hiring requests</div>
            ...
            <div class="tableWrap">
                <table>
                    ...
                    <tr>
                        ...
                        <th>Contact / Email</th>
                        <th>Organization</th>
                        <th>Resources &amp; salary package</th>
                        <th>Date</th>
                        <th>Action</th>
                    </tr>
                    </thead>
                    ...
                    <?php if ($result && $result->num_rows > 0): ?>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td class="detail"><?= htmlspecialchars($row['resources_salary'] ?? '') ?></td>
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="6">No hiring assistance enquiries yet.</td></tr>
                    <?php endif; ?>
                    </tbody>
                </table>
            </div>
